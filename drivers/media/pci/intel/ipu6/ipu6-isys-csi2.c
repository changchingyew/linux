...
s64 ipu6_isys_csi2_get_link_freq(struct ipu6_isys_csi2 *csi2)
{
	...
	s64 ret = v4l2_get_link_freq(ext_sd->ctrl_handler, 0, 0);
	if (ret < 0)
	{
	...
	printk("NKW %s: Link frequency: %lld Hz\n", __func__, ret);
	return ret;

#if 0
	if (ext_sd->ctrl_handler) {
			printk("NKW %s get link freq from ext_sd %s\n",__func__, ext_sd->name);
			return v4l2_get_link_freq(ext_sd->ctrl_handler, 0, 0);
	}
	else if (src_pad) {
			printk("NKW %s get link freq from src_pad %s\n",__func__, src_pad->entity->name);
